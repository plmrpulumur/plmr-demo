(function (root) {
  ...
  const SCHEMA = 'plmr-bio-rise-technical2d-projection-v14.23';
  const PRODUCT_ID = 'P3DV_ECO_BIOCLIMATIC';
  const PRODUCT_GROUP = 'bio-rise';
  // Eco-Bioclimatic projection dimensions mirror the canonical Bio-Rise product sections.
  // Module/post/frame topology is always produced by P3DVBioRiseMultiPosition.
  const PROFILE = Object.freeze({
    post: Object.freeze({ x: 150, z: 100 }),
    beam: Object.freeze({ vertical: 218, thickness: 100 }),
  ...
  function buildLayout(model) {
    const engine = root.P3DVBioRiseMultiPosition;
    if (!engine || typeof engine.build !== 'function') throw new Error('BIO_RISE_CANONICAL_MULTI_POSITION_ENGINE_MISSING');
  ...
  function bioPanelProjection(module, model) {
    ...
    const rearT = zSign * rearOuterZ, frontT = zSign * frontOuterZ;
    ...
    const firstMaxT = frontT - 216;
    const edges = [];
    for (let i = 0; i < panelCount; i += 1) edges.push({ panelIndex: i, z: zSign * (firstMaxT - (panelCount - 1 - i) * 200) });
    return { zSign, panelCount, panelLength, centerX, opened: Boolean(model && model.panelMasterOpen), edges };
  }

  function addBioTopStructure(entities, model, layout) {
    const frontBackThickness = Math.max(20, num(model.beamSection && model.beamSection.thickness, PROFILE.beam.thickness));
    ...
    (Array.isArray(layout.beams) ? layout.beams : []).forEach(beam => {
      if (beam.kind === 'width') {
        ...
        const z = num(beam.z, 0) + inward * frontBackThickness / 2;
        entities.push(rect(num(beam.x, 0) - num(beam.length, 0) / 2, z - frontBackThickness / 2, num(beam.length, 0), frontBackThickness, 'frame-profile', { productKind: 'eco-bioclimatic', beamId: beam.id, moduleIndex: beam.moduleIndex, row: beam.row }));
      } else {
        entities.push(rect(num(beam.x, 0) - sideThickness / 2, num(beam.z, 0) - num(beam.length, 0) / 2, sideThickness, num(beam.length, 0), 'frame-profile', { productKind: 'eco-bioclimatic', beamId: beam.id, moduleIndex: beam.moduleIndex, side: beam.side }));
      }
    });
    (Array.isArray(layout.modules) ? layout.modules : []).forEach(module => {
      const length = Math.max(1, num(module.gutterLength, num(module.depth, 0) - 100));
      const centerZ = num(module.gutterCenterZ, num(module.centerZ, 0));
      entities.push(rect(num(module.leftGutterOuterX, 0), centerZ - length / 2, Math.max(1, num(module.leftGutterInnerX, 0) - num(module.leftGutterOuterX, 0)), length, 'gutter-profile', { productKind: 'eco-bioclimatic', moduleIndex: module.moduleIndex, gutterSide: 'left' }));
      entities.push(rect(num(module.rightGutterInnerX, 0), centerZ - length / 2, Math.max(1, num(module.rightGutterOuterX, 0) - num(module.rightGutterInnerX, 0)), length, 'gutter-profile', { productKind: 'eco-bioclimatic', moduleIndex: module.moduleIndex, gutterSide: 'right' }));
    });
  }
  ...
  function buildTopView(model, layout, envelope) {
    ...
    addBioTopStructure(entities, model, layout);
    modules.forEach((module, index) => {
      const panel = bioPanelProjection(module, model);
      ...
      const pz0 = zs.length ? Math.min(...zs) : Math.min(num(module.rearOuterZ, 0), num(module.frontOuterZ, 0));
      const pz1 = zs.length ? Math.max(...zs) : Math.max(num(module.rearOuterZ, 0), num(module.frontOuterZ, 0));
      entities.push(rect(panelMinX, pz0, panel.panelLength, Math.max(1, pz1 - pz0), 'panel-zone', { moduleIndex: index, rowIndex: module.rowIndex, canonicalPanelProjection: true }));
      panel.edges.forEach(item => entities.push(line(panelMinX, item.z, panelMaxX, item.z, 'roof-panel', { moduleIndex: index, rowIndex: module.rowIndex, panelIndex: item.panelIndex, tilted: panel.opened })));
      if (panel.opened) entities.push(label(panel.centerX, pz0 + Math.max(20, (pz1 - pz0) * 0.08), 'LAMELLER AÇIK', 'product-state-label', { moduleIndex: index, state: 'TILT_OPEN' }));
      entities.push(label((num(module.outerMinX, panelMinX) + num(module.outerMaxX, panelMaxX)) / 2, (num(module.rearOuterZ, pz0) + num(module.frontOuterZ, pz1)) / 2, `M${index + 1}`, 'module-label', { moduleIndex: index, rowIndex: module.rowIndex }));
    });
    ...
    const width = envelope.maxX - envelope.minX, depth = envelope.maxZ - envelope.minZ;
  ...
  function bioVisibleModules(layout, side, envelope) {
    const modules = Array.isArray(layout.modules) ? layout.modules : [];
  ...
  function addBioElevationStructure(entities, model, layout, envelope, side, clearHeight, beamHeight) {
    const beams = Array.isArray(layout.beams) ? layout.beams : [];
    if (side === 'rear' || side === 'front') {
      const row = side === 'rear' ? 'rear' : 'front';
      beams.filter(beam => beam.kind === 'width' && beam.row === row).forEach(beam => entities.push(rect(num(beam.x,0)-num(beam.length,0)/2,clearHeight,num(beam.length,0),beamHeight,'frame-profile',{productKind:'eco-bioclimatic',beamId:beam.id,moduleIndex:beam.moduleIndex,side})));
    } else {
      const targetX = side === 'left' ? envelope.minX : envelope.maxX;
      beams.filter(beam => beam.kind !== 'width' && Math.abs(num(beam.x,0)-targetX)<=180).forEach(beam => {
        ...
        entities.push(rect(Math.min(start,end),clearHeight,Math.abs(end-start),beamHeight,'frame-profile',{productKind:'eco-bioclimatic',beamId:beam.id,side}));
      });
      bioVisibleModules(layout,side,envelope).forEach(module=>{
        const len=Math.max(1,num(module.gutterLength,num(module.depth,0)-100)), center=num(module.gutterCenterZ,num(module.centerZ,0));
        entities.push(rect(center-len/2,clearHeight,len,Math.min(98,beamHeight),'gutter-profile',{productKind:'eco-bioclimatic',moduleIndex:module.moduleIndex,gutterSide:side}));
      });
    }
  }

  function addBioElevationPanels(entities, model, layout, envelope, side, clearHeight) {
    const y=clearHeight+61, modules=bioVisibleModules(layout,side,envelope);
    if(side==='left'||side==='right') modules.forEach(module=>{
      const panel=bioPanelProjection(module,model);
      panel.edges.forEach(item=>{
        if(panel.opened) entities.push(line(item.z-8,y,item.z+22,y+105,'roof-panel',{moduleIndex:module.moduleIndex,panelIndex:item.panelIndex,side,tilted:true}));
        else entities.push(line(item.z-45,y,item.z+45,y,'roof-panel',{moduleIndex:module.moduleIndex,panelIndex:item.panelIndex,side,tilted:false}));
      });
    }); else modules.forEach(module=>{const panel=bioPanelProjection(module,model);entities.push(line(panel.centerX-panel.panelLength/2,y,panel.centerX+panel.panelLength/2,y,'roof-panel',{moduleIndex:module.moduleIndex,side,edgeProjection:true,tilted:panel.opened}));});
  }
  ...
  function buildElevation(model, layout, envelope, side, contract) {
    ...
    const clearHeight = Math.max(1, height - beamHeight), entities=[];
    entities.push(rect(horizontalMin,0,horizontalMax-horizontalMin,height,'elevation-envelope',{side}));
    addBioElevationStructure(entities,model,layout,envelope,side,clearHeight,beamHeight);
    addBioElevationPanels(entities,model,layout,envelope,side,clearHeight);
    elevationPosts(layout,model,side,envelope).forEach(post=>{const section=postSection(model,post);const center=horizontalAxis==='x'?num(post.x,0):num(post.z,0);const postWidth=horizontalAxis==='x'?section.x:section.z;entities.push(rect(center-postWidth/2,0,postWidth,Math.max(1,num(post.height,height)),'post',{postId:post.id,shared:Boolean(post.sharedBoundary||post.sharedAcrossRows)}));});
    const modules=Array.isArray(layout.modules)?layout.modules:[];
    const boundaries=horizontalAxis==='x'
      ?Array.from(new Set(modules.flatMap(m=>[num(m.outerMinX,NaN),num(m.outerMaxX,NaN)]).filter(Number.isFinite).map(v=>Math.round(v*1000)/1000))).sort((a,b)=>a-b)
      :Array.from(new Set(modules.flatMap(m=>[num(m.rearOuterZ,NaN),num(m.frontOuterZ,NaN)]).filter(Number.isFinite).map(v=>Math.round(v*1000)/1000))).sort((a,b)=>a-b);
    ...
    addFacadeProfiles(entities,model,facadeId,horizontalMin,horizontalMax,clearHeight); addInteractionZones(entities,model,contract,side);
    entities.push(dim('x',horizontalMin,horizontalMax,-Math.max(180,height*.06),`${Math.round(horizontalMax-horizontalMin)} mm`,{kind:'overall-horizontal',side}));
    entities.push(dim('y',0,height,horizontalMax+Math.max(180,(horizontalMax-horizontalMin)*.04),`${Math.round(height)} mm`,{kind:'overall-height',side}));
  ...
  function build(snapshot, context) {
    ...
    if (!source || source.schema !== 'p3dv-host-snapshot-v1' || !source.modelState) throw new Error('BIO_RISE_TECHNICAL2D_SNAPSHOT_INVALID');
    const model = clone(source.modelState);
    if (model.productGroup !== PRODUCT_GROUP) throw new Error('BIO_RISE_TECHNICAL2D_PRODUCT_MISMATCH');
    const layout = buildLayout(model);
    if (!layout || !layout.valid) {
      const errors = layout && Array.isArray(layout.errors) ? layout.errors.slice() : ['BIO_RISE_LAYOUT_INVALID'];
  ...
  root.PulumurBioRise2DAdapter = api;
